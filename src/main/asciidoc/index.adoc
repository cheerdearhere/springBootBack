= test REST API Guide
dreamIk Heimer (dream-ik89@naver.com);
:doctype: book
:icons: font
...
| `PUT`
| 기존 resouce를 수정할 때 사용

| `PATCH`
| 기존 resouce의 일부를 수정할 때 사용

| `DELETE`
| 기존 resouce를 삭제할 떄 사용
|===
...
operation::query-events[snippets='response-headers,response-fields,curl-request,http-response,links']
...
!===
|PathValiable|Type|Description
|`+id+`
|`+Number+`
|event id
!===
operation::get-event[snippets='curl-request,http-request,http-response,links']
...
operation::update-event[snippets='request-fields,curl-request,http-request,request-headers,http-response,response-headers,response-fields,links']